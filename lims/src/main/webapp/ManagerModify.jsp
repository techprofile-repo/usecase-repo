...
rs=DBConn.executeQuery("select * from branchmgr where branchmgrid=" + mgrid );
if(rs.next())
{
  ...
  <table width="539" border="3" align="center" cellpadding="2" cellspacing="2">
	...
	%>
      ...
      <td><select name="mbname" id="mbname">
			  ...
			  <%
rs1=DBConn.executeQuery("select branchname from branch where branchid!=(select branchid from branch where branchname='"+branchname +"')");
		  ...
		  <%  
			   rs2=DBConn.executeQuery("select password from login where userid="+ mgrid );
			   if(rs2.next())
			   {%>
      <td>Password</td>
