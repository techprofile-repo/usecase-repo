...
con=DBConn.getConnection();
 rs=DBConn.executeQuery("select * from agents where agentid="+ agentid);
...
%>
  ...
  <table width="572" border="0" align="center" cellpadding="2" cellspacing="2">
	...
	rs1=DBConn.executeQuery("select branchmgrname from branchmgr where branchmgrid="+ branchmgrid);
	if(rs1.next())
	{
    ...
    <tr> 
      ...
      <td height="30"><select name="mbname" id="select">
          ...
          <% rs3=DBConn.executeQuery("select branchmgrname from branchmgr where branchmgrid!="+ branchmgrid);
		while(rs.next())
		{
		%>
		  ...
		  <%  
			   rs2=DBConn.executeQuery("select password from login where userid="+ agentid );
			   if(rs2.next())
			   {%>
